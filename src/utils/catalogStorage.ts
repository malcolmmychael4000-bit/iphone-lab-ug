 ...
 */
export function sanitizeImageUrl(url: string | undefined | null, fallbackPartId?: string, tier?: 'incell' | 'oled' | 'main'): string {
  ...
  if (trimmed.startsWith('/uploads/') || trimmed.startsWith('/images/') || trimmed.startsWith('/')) {
    return trimmed;
  }
...
export function normalizePartImageFields(part: PartProduct): PartProduct {
  const imageUrl = pickFirstNonEmptyString(part.imageUrl, part.image_url);
  const incellImageUrl = pickFirstNonEmptyString(part.incellImageUrl, part.incell_image_url);
  const oledImageUrl = pickFirstNonEmptyString(part.oledImageUrl, part.oled_image_url);

  return {
    ...part,
    imageUrl: sanitizeImageUrl(imageUrl, part.id, 'main'),
    image_url: sanitizeImageUrl(imageUrl, part.id, 'main'),
    incellImageUrl: sanitizeImageUrl(incellImageUrl, part.id, 'incell'),
    incell_image_url: sanitizeImageUrl(incellImageUrl, part.id, 'incell'),
    oledImageUrl: sanitizeImageUrl(oledImageUrl, part.id, 'oled'),
    oled_image_url: sanitizeImageUrl(oledImageUrl, part.id, 'oled'),
  };
}
...
export function getStoredParts(): PartProduct[] | null {
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      const sanitized = parsed.map((p: PartProduct) => normalizePartImageFields(p));
...
export async function hydrateCatalogFromIdb(): Promise<PartProduct[] | null> {
  try {
    ...
    if (idbParts && Array.isArray(idbParts) && idbParts.length > 0) {
      const sanitized = idbParts.map((p: PartProduct) => normalizePartImageFields(p));
      memoryCache = sanitized;
...
export function saveStoredParts(parts: PartProduct[]): void {
  try {
    const safeParts = parts.map((p) => normalizePartImageFields(p));
...
export function mergeWithStoredParts(serverParts: PartProduct[]): PartProduct[] {
  ...
  const merged = serverParts.map((sp) => {
    ...
    if (!custom) return normalizePartImageFields(sp);
    ...
    return normalizePartImageFields({
      ...sp,
      ...custom,
      imageUrl: finalImg,
      image_url: finalImg,
  ...
  return [...merged, ...additionalCustomParts.map((part) => normalizePartImageFields(part))];
}
